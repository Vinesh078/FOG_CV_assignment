...
- **eps=0.55 is hand-picked, not fit.** With the real dataset and even partial ground
  truth, I'd sweep eps and pick the value that maximizes cluster purity, and I'd rather
  do that than guess a second time.
- **No re-identification across sessions.** Every run clusters from scratch; there's no
  persisted "known identities" database to match new photos against later. Straightforward
  to add (store cluster centroids, match new faces against them with a distance
  threshold) but out of scope for a batch clustering task.
- **Confidence is similarity, not calibrated probability**, as above — flagging it again
  because it's the most likely thing to get over-read at a glance.
- **DBSCAN can split one person into two clusters** under extreme lighting/pose swings,
  since it's density-based rather than doing any cross-cluster identity merging pass.
  A second pass that merges clusters whose centroids are within some looser threshold
  would help, at the cost of more false merges elsewhere — precision/recall tradeoff,
  not a free win.
- **Didn't test at real scale.** MTCNN + InceptionResnetV1 are CPU-friendly, but I
  haven't run this against a multi-thousand-image dataset, so I don't have real
  throughput numbers to report beyond the ~300ms/face I saw on my 5-face sanity set.

## Demo video

`[Google Drive link — add after recording]`

Script I used: upload a handful of photos with 2-3 people, some appearing multiple
times and at least one solo shot with no group overlap → click "Cluster faces" →
walk through the clusters that come back, calling out one high-confidence match and
the unmatched bucket if one appears.
